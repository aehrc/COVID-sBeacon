...
resource null_resource upload {
  ...
  provisioner local-exec {
      command = "./upload.sh ${aws_s3_bucket.website_bucket.id}  ${aws_cloudfront_distribution.platform_distribution.id} ${var.beacon_api_url} ${var.login} ${var.production} 'https://'${aws_cloudfront_distribution.platform_distribution.domain_name}"
      working_dir = path.module
    }
